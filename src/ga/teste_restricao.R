for(tour in tours){
    ...
    if(length(percurso) != 0){ 
      ...
      for (p in c(1:length(percurso))){
        ...
        if((carga_atual==0)&& (stock_atual==0) && (p!=length(percurso))){  
          #return(0)  
          cat("O deposito ", i, " não tem stock para o percurso do camiao\n")
        }
        ...
        if(procura[p] == 0 ){ 
          #return(0)  
          cat("O camiao do deposito ", i, " visita clientes já atendidos por outros. procura=0.\n")
        }
  ...
  if((sum(cli_visitados)!=nr_clientes) || (sum(procura[p])!=0)){  
    #return(0)  
    cat("Os percursos do cromossoma nao cobrem todos os clientes ou não satisfaz a procura dos clientes. \n")
  }else{ 
    #return(-custo)
    custo
  }
